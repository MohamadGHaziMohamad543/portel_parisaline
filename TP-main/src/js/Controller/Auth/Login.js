       ...
       //  console.log($cookies.remove('PUTR'));
        ...
        let Login=(res)=>{
            if(res.tempPass)
            {
                ...
                $cookies.put('PUTR', JSON.stringify({token:res.token,Did:res.user.dentalCenterId,name:res.user.nameDoctor,image:res.user.logo,phone:res.user.phoneNumber,email:res.user.email})); //PUA Parisaline User ACCountant
                $scope.$apply(()=>{
