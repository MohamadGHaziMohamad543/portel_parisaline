var CONT_CreateOrder = function ($scope, $location, $routeParams, $rootScope) {
    this.$onInit = function () {
        ...
        HTTPs.POST(CONFIG.serverUrl + "/GISTL", { id: this.idPatient }).then(res => {
            ...
            if (res.message == 2000) {
                $scope.$apply(() => {
                    ...
                    $scope.langCode = res.languages[0].DLangCode;
                    $scope.showLanguage  = res.showLanguage;
                });
            }
        });
        $scope.DataAdress = [];
        ...
        $scope.Create = () => {
            ...
            let Param = {
                ...
                langCode: document.querySelector('#langCode').value,
            };
        ...
        var newOrder = () => {
            ...
            $scope.getAllValues = () => {
                ...
                if (document.querySelector('input[name="Impressions"]:checked').value === 2) {
                    let LowerStl = document.querySelector('.boxSTL #LowerStl').getAttribute('srctemp');
                    let UpperStl = document.querySelector('.boxSTL #UpperStl').getAttribute('srctemp');
                    let OcclusionStl = document.querySelector('.boxSTL #OcclusionStl').getAttribute('srctemp');
                    if (!LowerStl) {
                        document.querySelector('.boxSTL #LowerStl').style.border = "4px solid #e65959";
                        ErrorNumber++;
                    }
                    if (!UpperStl) {
                        document.querySelector('.boxSTL #UpperStl').style.border = "4px solid #e65959";
                        ErrorNumber++;
                    }
                    if (!OcclusionStl) {
                        document.querySelector('.boxSTL #OcclusionStl').style.border = "4px solid #e65959";
                        ErrorNumber++;
                    }
                }
                ...
                for (let i = 0; i < SpacesIPRElements.length; i++) {
                    PlaceButtonCutsOnIPRVal.push({ name: PlaceButtonCutsOnIPRElements[i].id.split('PlaceButtonCutsOn')[1], value: PlaceButtonCutsOnIPRElements[i].checked })
                }
                valueCreateOeder['PlaceButtonCutsOn'] = PlaceButtonCutsOnIPRVal;
