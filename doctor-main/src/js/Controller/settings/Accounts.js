...
var CONT_Accounts = function ($scope, $location, $routeParams) {
    $scope.Status = 0;
    ...
    var displedForm = () => {
        ...
        $("#ClinicName").attr("disabled", "disabled");
        $("#langCode").attr("disabled", "disabled");
    }
    var EnableForm = () => {
        ...
        $("#ClinicName").removeAttr("disabled");
        $("#langCode").removeAttr("disabled");
    }
    ...
    $scope.getAll = () => {
        HTTPs.POST(CONFIG.serverUrl + "/GPD", {}).then(res => {
            res = JSON.parse(res);
            console.log(res, 'Result Data Languages');
            if (res.error == 0) {
                ...
                $scope.$apply(() => {
                    ...
                    $scope.langCode = res.data.langCode.toString();
    ...
    $scope.Save = () => {
        ...
        HTTPs.POST(CONFIG.serverUrl + "/UPD", dataF, valueForm).then(res => {
            res = JSON.parse(res);
            if (res.data) {
                $scope.$apply(() => {
                    $scope.LoaderForm = false;
