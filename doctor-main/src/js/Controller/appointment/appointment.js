var CONT_Appointment = function ($scope, $location, $routeParams) {
    this.$onInit = function () {
        ...
        function getStatusAppentment(status) {
            ...
            return `<span class="Draft" style="background: ${color};text-shadow: 1px 1px 2px #000;color:#fff;width: 100%;display: block;padding: 6px 6px;">${st}</span>`
        }
        $scope.data = [];
        $scope.CuurancyDefault = -1;
        $scope.GetAll = () => {
            tabel.clear().draw();
            HTTPs.POST(CONFIG.serverUrl + "/Appointment/GetAll", { filters: this.filters, dateFilter: new Date().toISOString().split("T")[0] })
                .then(res => {
                    res = JSON.parse(res);
                    $scope.$apply(() => {
                        $scope.data = res.data;
                    ...
                    for (let i = 0; i < res.data.length; i++) {
                        ...
                        "><i class="mdi mdi-account-edit"></i>Edit</button></main>`,
                        ]).draw();
                    }
                });
        }
