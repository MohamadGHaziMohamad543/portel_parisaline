            ...
            HTTPs.POST(CONFIG.serverUrl+"/GAD",{}).then(res=>{
                ...
                res.forEach(row => {
                    tabel.row.add([
                        ...
                        `<a style="width:100%;text-align:center;display: block;">${row.CountPatients}</a>`,
                        `<a style="width:100%;text-align:center;display: block;">${row.CountInvoices}</a>`,
                        `<a style="width:100%;text-align:center;display: block;">${getStatusDoctors(row.status)}</a>`,
