        ...
        $scope.recoveryPassword=()=>{
            ...
            let Login=(res)=>{
                if(res.tempPass)
                {
                    ...
                    $cookies.put('PUC', JSON.stringify({token:res.token,Did:res.user.dentalCenterId,name:res.user.nameDoctor,image:res.user.logo,phone:res.user.phoneNumber,email:res.user.email})); //PUA Parisaline User Doctor
                    $scope.$apply(()=>{
