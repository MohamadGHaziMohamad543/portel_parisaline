$ANContreoller.Auth_Login={
    FUN:['$scope','$cookies','$location',function($scope,$cookies,$location){
       ...
       //  console.log($cookies.remove('PUC'));
        if($cookies.get('PUC'))
        {
           // $location.path('/');
        }
        ...
        let Login=(res)=>{
            if(res.tempPass)
            {
                ...
                $cookies.put('PUC', JSON.stringify({token:res.token,Did:res.user.dentalCenterId,name:res.user.nameDoctor,image:res.user.logo,phone:res.user.phoneNumber,email:res.user.email})); //PUA Parisaline User ACCountant
                $scope.$apply(()=>{
