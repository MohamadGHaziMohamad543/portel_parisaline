...
export class LabProfileComponent implements OnInit {
  ...
  constructor(
    ...
    private labServe:LaboratorysService,
 ...
 ngOnInit() {
  ...
  this.labServe.getById(this.id).subscribe(res=>{
    console.log(res);
    this.Lab=res[0];
 ...
 changePasswordForLab(password:string)
 {
   this.labServe.chpass(password,this.id).subscribe(res=>{
     if(res.message==2001)
     {
