$ANContreoller.Supervisors={
    ...
    var time = $(this).attr('data-time')?$(this).attr('data-time'):1200; //default is 1200
        ...
        "createdRow": function( row, data, dataIndex ) {
            ...
            $(row).find('[data-plugin="switchery"]').on("change",(e)=>{
                ...
                HTTPs.POST(CONFIG.serverUrl+"/S/USS",{id:$(row).find('[data-plugin="switchery"]').attr("data-id"),status:status}).then(res=>{
                res=JSON.parse(res);
    ...
    $scope.GetAll=()=>{
        HTTPs.POST(CONFIG.serverUrl+"/S/GAS",{}).then(res=>{
            res=JSON.parse(res).data;
            tabel.clear().draw();
            res.forEach(row => {
                tabel.row.add([
                    `<a class="text-left" style="display: block;"><img src="${CONFIG.serverImageUrl+row.logo}">${row.nameSupervisor}</a>`,
    ...
    $scope.Edit=(id)=>{
        $scope.typeForm="Edit";
        HTTPs.POST(CONFIG.serverUrl+"/S/GSI",{id:id})
        .then(res=>{
            res=JSON.parse(res);
            idAccount=id;
            if(res.error==0)
            {
                $scope.$apply(()=>{
                    document.getElementById("nameSupervisor").value=res.data[0].nameSupervisor;
    ...
    input.addEventListener("countrychange", function(e) {
        ...
        $scope.saveAccount=()=>{
            ...
            l.start();
            HTTPs.POST(CONFIG.serverUrl+"/S/UPS",dataF,valueForm).then(res=>{
                res=JSON.parse(res);
                if(res.error==0)
                {
        ...
        $scope.CreateAccount=()=>{
            ...
            dataF.append("fileData",FileUpload);
            l.start();
            HTTPs.POST(CONFIG.serverUrl+"/S/CS",dataF,valueForm).then(res=>{
                res=JSON.parse(res);
                if(res.message==2000)
                {
    ...
    Router:{
        Url:"/Supervisors",
        Templete:"Views/SuperVisors/SuperVisors.html",
        Render:[
