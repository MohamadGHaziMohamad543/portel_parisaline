       ...
       //  console.log($cookies.remove('PUCL'));
        ...
        let Login=(res)=>{
            if(res.tempPass)
            {
                ...
                $cookies.put('PUTERCL', JSON.stringify({token:res.token,Did:res.user.dentalCenterId,name:res.user.nameDoctor,image:res.user.logo,phone:res.user.phoneNumber,email:res.user.email})); //PUA Parisaline User ACCountant
                $scope.$apply(()=>{
