...
$ANContreoller.Auth_Signup={
    FUN:['$scope','$cookies',function($scope,$cookies){
        ...
        let Login=(res)=>{
            ...
            $cookies.put('PUCL', JSON.stringify({token:res.token,Did:res.user.dentalCenterId,name:res.user.nameDoctor,image:res.user.logo,phone:res.user.phoneNumber})); //PUA Parisaline User Doctor
            $scope.$apply(()=>{
        ...
        let regestry=(res)=>{
            if(res.tempPass)
            {
                ...
                CONFIG.Did=res.user.dentalCenterId;
                $cookies.put('PUCL', JSON.stringify({token:res.token,Did:res.user.dentalCenterId,name:res.user.nameDoctor,image:res.user.logo,phone:res.user.phoneNumber,email:res.user.email})); //PUA Parisaline User Doctor
                $scope.$apply(()=>{
                    $scope.ErrorMessagss="Authentication completed successfully, you will be redirected to the control panel";
                    setTimeout(() => {
                        $('#email').css('border','1px dashed green'); 
