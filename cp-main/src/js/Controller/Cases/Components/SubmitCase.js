    ...
    this.$onInit = function () {
        ...
        $scope.addTask = () => {
            ...
            for (var i = 0; i < $scope.Tasks.length; i++) {
                ...
                Tasks[i] = { patientId: $scope.Tasks[i].patientId, userId: $scope.Tasks[i].userId, usertype: $scope.Tasks[i].usertype, action: $scope.Tasks[i].action, Start: $scope.Tasks[i].Start, End: $scope.Tasks[i].End, endTask: $scope.Tasks[i].endTask, commentAdmin: $scope.Tasks[i].commentAdmin, commentUser: $scope.Tasks[i].commentUser, rejectStartPoint: $scope.Tasks[i].rejectStartPoint }
            }
            if ($("#rejectStartPoint").is(":checked")) {
                rejectStartPoint = 1;
            }
            Tasks.splice(index, 0, { patientId: this.idPatient, userId: $('#UserId').val(), usertype: $("#usertype").val(), action: $("#action").val(), Start: Start, End: null, endTask: null, commentAdmin: $("#CommentInput").val(), commentUser: null, rejectStartPoint: rejectStartPoint });
        ...
        $scope.UpdateCommentAdmin = (numberIndex) => {
            $scope.Loader = true;
            let id = $scope.Tasks.find(x => x.numberIndex == numberIndex).id;
            let comment = prompt(`Change the note (${$scope.Tasks.find(x => x.numberIndex == numberIndex).commentAdmin}) to`);
            if (comment) {
                HTTPs.POST(CONFIG.serverUrl + "/TAS/UCA", { id: id, commentAdmin: comment }).then(res => {
                    res = JSON.parse(res);
                    if (res.error == 0) {
                        $scope.$apply(() => {
                            $scope.GetTasks();
            ...
            HTTPs.POST(CONFIG.serverUrl + "/CS/GASS", {}).then(res => {
                ...
                if (res.error == 0) {
                    $scope.$apply(() => {
                        ...
                        document.getElementById('casesStatusElementCases').innerHTML = Item;
                    });
