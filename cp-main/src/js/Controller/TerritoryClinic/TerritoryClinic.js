$ANContreoller.TerritoryClinic = {
    FUN: ['$scope', '$location', '$routeParams', function ($scope, $location, $routeParams) {
        ...
        var tabel = $('#scroll-vertical-datatable').DataTable({
            ...
            "createdRow": function (row, data, dataIndex) {
                ...
                $(row).find('[data-plugin="switchery"]').on("change", (e) => {
                    ...
                    HTTPs.POST(CONFIG.serverUrl + "/TER/USA", { id: $(row).find('[data-plugin="switchery"]').attr("data-id"), status: status }).then(res => {
                        res = JSON.parse(res);
        ...
        $scope.GetAll = () => {
            HTTPs.POST(CONFIG.serverUrl + "/TER/GAA", {}).then(res => {
                res = JSON.parse(res).data;
                tabel.clear().draw();
                res.forEach(row => {
                    tabel.row.add([
                        `<a class="text-left" style="display: block;"><img src="${CONFIG.serverImageUrl + row.logo}">${row.nameAcount}</a>`,
        ...
        $scope.ResponsibleTer = [];
        $scope.addTer = () => {
            let id = document.getElementById('ResponsibleTer').value;
            if (!$scope.ResponsibleTer.find(x => x.id === id)) {
                let text = document.getElementById('ResponsibleTer').options[document.getElementById('ResponsibleTer').selectedIndex].text
                $scope.ResponsibleTer.push({ id: id, name: text });
            }
        }

        $scope.RemoveTer = (key) => {
            $scope.ResponsibleTer.splice(key, 1);
        }

        $scope.Edit = (id) => {
            $scope.typeForm = "Edit";
            HTTPs.POST(CONFIG.serverUrl + "/TER/GAI", { id: id })
                .then(res => {
                    res = JSON.parse(res);
                    idAccount = id;
                    if (res.error == 0) {
                        $scope.$apply(() => {
                            document.getElementById("nameAcount").value = res.data[0].nameAcount;
                            ...
                            if (res.data[0].ResponsibleCountries) {
                                $scope.ResponsibleCountries = JSON.parse(JSON.parse(res.data[0].ResponsibleCountries));
                            }
                            else {
                                $scope.ResponsibleCountries = [];
                            }
                            $scope.changeCountry(res.data[0].cityId.toString());
                            document.getElementById('PatientInformation').checked = res.data[0].PatientInformation;
                            document.getElementById('PatientRecords').checked = res.data[0].PatientRecords;
                            document.getElementById('SelectPackage').checked = res.data[0].SelectPackage;
                            document.getElementById('CreateOrder').checked = res.data[0].CreateOrder;
                            document.getElementById('AdditionalInstructions').checked = res.data[0].AdditionalInstructions;
                            document.getElementById('TretmentPlan').checked = res.data[0].TretmentPlan;
                            document.getElementById('TasksForCase').checked = res.data[0].TasksForCase;
                            let ResponsibleTer =res.data[0].ResponsibleTer;
                            try {
                                if (typeof ResponsibleTer == "string") {
                                    ResponsibleTer = JSON.parse(ResponsibleTer);
                                }
                                if (typeof ResponsibleTer == "string") {
                                    ResponsibleTer = JSON.parse(ResponsibleTer);
                                }
                            ...
                            $scope.ResponsibleTer = ResponsibleTer;
        ...
        $scope.ResponsibleCountries = [];
        $scope.addCoutry = () => {
            let id = document.getElementById('countryResposable').value;
            if (!$scope.ResponsibleCountries.find(x => x.id === id)) {
                let text = document.getElementById('countryResposable').options[document.getElementById('countryResposable').selectedIndex].text
                $scope.ResponsibleCountries.push({ id: id, name: text });
            }
        }
        $scope.RemoveCaountry = (key) => {
            $scope.ResponsibleCountries.splice(key, 1);
        }
        $scope.saveAccount = () => {
            ...
            let arrayCountey = [];
            $scope.ResponsibleCountries.forEach(res => {
                arrayCountey.push({ id: res.id, name: res.name });
            });
            
            let ResponsibleTer = [];
            $scope.ResponsibleTer.forEach(res => {
                ResponsibleTer.push({ id: res.id, name: res.name });
            });

            valueForm['ResponsibleCountries'] = JSON.stringify(arrayCountey);
            valueForm['PatientInformation'] = document.getElementById('PatientInformation').checked;
            valueForm['PatientRecords'] = document.getElementById('PatientRecords').checked;
            valueForm['SelectPackage'] = document.getElementById('SelectPackage').checked;
            valueForm['CreateOrder'] = document.getElementById('CreateOrder').checked;
            valueForm['AdditionalInstructions'] = document.getElementById('AdditionalInstructions').checked;
            valueForm['TretmentPlan'] = document.getElementById('TretmentPlan').checked;
            valueForm['TasksForCase'] = document.getElementById('TasksForCase').checked;
            valueForm['ResponsibleTer'] = JSON.stringify(ResponsibleTer);
            ...
            l.start();
            HTTPs.POST(CONFIG.serverUrl + "/TER/UPA", dataF, valueForm).then(res => {
                res = JSON.parse(res);
                if (res.error == 0) {
        ...
        $scope.CreateAccount = () => {
            ...
            dataF.append("fileData", FileUpload);
            l.start();
            HTTPs.POST(CONFIG.serverUrl + "/TER/CA", dataF, valueForm).then(res => {
                res = JSON.parse(res);
                if (res.message == 2000) {
        ...
        //TerritoryManagers
        $scope.TerritoryManagers;
        $scope.getAllTerritoryManagers = () => {
            HTTPs.POST(CONFIG.serverUrl + "/TEM/GATM", {}).then(res => {
                res = JSON.parse(res);
                if (res.error == 0) {
                    $scope.$apply(() => {
                        $scope.TerritoryManagers = res.data;
                    });
                }
            });
        }
    ...
    Router: {
        Url: "/TerritoryClinic",
        Templete: "Views/TerritoryClinic/TerritoryClinic.html",
        Render: [
