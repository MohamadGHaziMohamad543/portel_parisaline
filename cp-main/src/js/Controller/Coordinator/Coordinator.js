$ANContreoller.Coordinator={
    ...
    var time = $(this).attr('data-time')?$(this).attr('data-time'):1200; //default is 1200
        ...
        "createdRow": function( row, data, dataIndex ) {
            ...
            $(row).find('[data-plugin="switchery"]').on("change",(e)=>{
                ...
                HTTPs.POST(CONFIG.serverUrl+"/C/USC",{id:$(row).find('[data-plugin="switchery"]').attr("data-id"),status:status}).then(res=>{
                res=JSON.parse(res);
    ...
    $scope.GetAll=()=>{
        HTTPs.POST(CONFIG.serverUrl+"/C/GAC",{}).then(res=>{
            res=JSON.parse(res).data;
            tabel.clear().draw();
            res.forEach(row => {
                tabel.row.add([
                    `<a class="text-left" style="display: block;"><img src="${CONFIG.serverImageUrl+row.logo}">${row.nameMediator}</a>`,
    ...
    input.addEventListener("countrychange", function(e) {
        ...
        $scope.saveAccount=()=>{
            ...
            l.start();
            HTTPs.POST(CONFIG.serverUrl+"/C/UPC",dataF,valueForm).then(res=>{
                res=JSON.parse(res);
                if(res.error==0)
                {
        ...
        $scope.CreateAccount=()=>{
            ...
            dataF.append("fileData",FileUpload);
            l.start();
            HTTPs.POST(CONFIG.serverUrl+"/C/CC",dataF,valueForm).then(res=>{
                res=JSON.parse(res);
                if(res.message==2000)
                {
    ...
    Router:{
        Url:"/Coordinators",
        Templete:"Views/Coordinator/Coordinator.html",
        Render:[
