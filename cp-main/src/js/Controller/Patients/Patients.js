$ANContreoller.Patients = {
  FUN: [
    ...
    function ($scope, $location, $cookies) {
      ...
      console.log('CONFIG.serverUrl + "/GAPS"', CONFIG.serverUrl + "/GAPS");
      ...
      var tabel = $("#scroll-vertical-datatable").DataTable({
        ...
        columns: [
          {
            ...
            render: (data, type, row) => {
              return `<a href="/Cases/${$CRID.TO(
                row.id.toString()
              )}" style="width:100%;text-align:center;display: block;"> <min style="background:${
                row.CountPatientByDoctor === 1 ? "#5b0404" : "#0000"
              }; color: #fff;    width: 50px; padding: 5px; transform: rotateZ(270deg); display: inline-block;font-size: 10px;height: 25px;">${
                row.CountPatientByDoctor === 1 ? "1st Case" : ""
              }</min> ${row.id}</a>`;
            },
          },
          {
            ...
            render: (data, type, row) => {
              return `<a href="/Cases/${$CRID.TO(
                row.type == 0
                  ? row.id.toString()
                  : row.refinementNumber.toString()
              )}" style="width:100%;text-align:center;display: block;">${
                row.type == 0
                  ? "New Patient"
                  : row.type == 1
                  ? "Refinement " + row.refinementNumber
                  : "Retainer " + row.refinementNumber
              }</a>`;
            },
          },
          {
          ...
          },
          {
            ...
            render: (data, type, row) => {
              return `<img src="${
                row.logo
                  ? row.logo.split("https://").length == 2
                    ? row.logo
                    : CONFIG.serverImageUrl + row.logo
                  : ""
              }">${row.nameDoctor}`;
            },
          },
          {
            data: "image",
            render: (data, type, row) => {
              return `<img src="${CONFIG.serverImageUrl + row.image}">${
          ...
          },
          {
            data: "caseStatus",
            render: (data, type, row) => {
              return `<a style="width:100%;text-align:center;display: block;">${getStatusCases(
                data
              )}</a>`;
            },
          },
          {
            data: "casesSubmitedDate",
            render: (data, type, row) => {
          ...
          {
            data: "parisalinePartner",
            render: (data, type, row) => {
              return `<a style="width:100%;text-align:center;display: block;">${
                row.parisalinePartner == 0 ? "NO" : "YES"
              }</a>`;
            },
          },
          {
            data: "dentalCenterName",
            render: (data, type, row) => {
          ...
          },
          {
            ...
            render: (data, type, row) => {
              return `<a class="btn btn-outline-primary btn-sm btnInfo" style="padding: 3px 20px;" infoid='${data}'">
                            ...
                            ${
                              row.TrackingNumber
                                ? ` <a class="btn btn-outline-primary btn-sm btnShipping" style="padding: 0px 0px;" infocountry='${row.countryName}' infoid='${row.TrackingNumber}'">
        ...
        drawCallback: function (e) {
          // var tableData = tabel.rows().data().toArray(); // Get all rows data as an array
          // console.log("Full Table Data in Table:", tableData);
          $(`.btnInfo`).click((e) => {
          ...
          $(`.btnShipping`).click((e) => {
            getAllShipping(
              $(e.currentTarget).attr("infoid"),
              $(e.currentTarget).attr("infocountry")
            );
          });
      ...
      var getStatusCases = (status) => {
        if (status == null) {
          ...
          return (
            '<span class="Draft" style="background: ' +
            color +
      ...
      var getAllShipping = (numberTrack, Country) => {
        ...
        HTTPs.POST(CONFIG.serverUrl + "/sh/GSA", {
