    ...
    FUN: ['$scope', '$location', '$routeParams', function ($scope, $location, $routeParams) {
        ...
        var tabel = $('#scroll-vertical-datatable').DataTable({
            ...
            "createdRow": function (row, data, dataIndex) {
                row.querySelector(".CloneTemplete").addEventListener("click", (e) => {
                    let idTemplete = e.currentTarget.getAttribute("dataid");
                    $scope.$apply(() => {
                        $scope.IdTmplete = idTemplete;
                        $scope.CloneModel();
                    })
                })
            }
            ...
            HTTPs.POST(CONFIG.serverUrl + "/TT/GATT", {}).then(res => {
                ...
                res.forEach(row => {
                    tabel.row.add([
                        ...
                        `<a class="text-left" style="display: block;">${row.typeSelect}</a>`,
                        `<a class="text-left" style="display: block;">${row.countryName}</a>`,
                        `<a class="text-left" style="display: block;">${row.cityName}</a>`,
                        `<a class="text-left" style="display: block;">${row.Doctor}</a>`,
                        `<a class="text-left" style="display: block;">${row.typeCase}</a>`,
                        `<a class="text-left" style="display: block;">
                                <a class="btn btn-outline-danger" href="TempleteTasks/${row.id}?TS=D${row.id}"><i class="fe-trash"></i></a>
                                <a class="btn btn-outline-info CloneTemplete" dataid="${row.id}"><i class="fa fa-clipboard"></i> Clone Template (${row.taskCase} Tasks)</a>
                        </a>`,
                    ]).draw();
                });
            });
        ...
        $scope.CloneModel = () => {
            ...
            idAccount = -1;
            $scope.TitleForm = "Clone Templete And Copy All Tasks";
            $('#FormModal input').val("");
            modal.open();
        }

        $scope.getDoctors = () => {
            HTTPs.POST(CONFIG.serverUrl + "/CC/GDoctor", {}).then(res => {
                res = JSON.parse(res);
                $scope.$apply(() => {
                    $scope.DoctorItem = res.data;
                });
            });
        }

        $scope.getCountrys = () => {
            HTTPs.POST(CONFIG.serverUrl + "/CC/GCU", {}).then(res => {
                res = JSON.parse(res);
                $scope.$apply(() => {
        ...
        $scope.getCitys = () => {
            HTTPs.POST(CONFIG.serverUrl + "/CC/GCI", {}).then(res => {
                res = JSON.parse(res);
                $scope.$apply(() => {
                    $scope.CitysItemFull = res.data;
        ...
        $scope.changeCountry = (id = null) => {
            $scope.CitysItem = $scope.CitysItemFull.filter(x => x.countryId == document.querySelector("#countryId").value);
            if (id) {
                $scope.City = id;
            }
            else {
        ...
        $scope.CloneTemplete = () => {
            ...
            valueForm['id'] = $scope.IdTmplete;
            $scope.LoaderForm = true;
            l.start();
            HTTPs.POST(CONFIG.serverUrl + "/TT/CTACT", valueForm).then(res => {
                res = JSON.parse(res);
                if (res.message == 2000) {
                    Tost.info("Created successfully", "Record added successfully", 3000);
                    Custombox.modal.close();
