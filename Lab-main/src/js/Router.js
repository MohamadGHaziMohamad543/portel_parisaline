    ...
    FUN: function ($rootScope) {
        return {
            ...
            link: function (scope, element, attrs) {
                attrs.changeName = true;
                if (attrs.nochangename == "true") {
                    attrs.changeName = false;
                }
                $rootScope.$on('updateUpload' + attrs.id + attrs.idpatient + attrs.staticname, (event, valueFile) => {
                    if (!tempFiles) {
                        renderFileUpload(valueFile.file, valueFile.type, attrs.changeName);
                    }
                    else {
                        UploadFile(valueFile.numberPrograss, '', '', valueFile.newPath, attrs.changeName);
                    }
                });
                ...
                function renderFileUpload(files, typeFileTemp = null, changeName) {
                    if (typeFileTemp) {
                        if (typeFileTemp) {
                        ...
                        if (tempFiles.length != 0) {
                            ...
                            if (ty.toLowerCase() == "png" || ty == "jpeg" || ty == "jpg") {
                                if (FileReader && tempFiles && tempFiles.length) {
                                    ...
                                    if (scope.$root.$$phase) {
                                        scope.$emit('uploadFiles', { id: attrs.id, fileName: tempFiles[0].name, filePrograss: 0, file: tempFiles[0], event: UploadFile, type: "image", idPatient: attrs.idpatient, changeName: attrs.changeName, staticname: attrs.staticname });
                                    }
                                    else {
                                        scope.$apply(() => {
                                            scope.$emit('uploadFiles', { id: attrs.id, fileName: tempFiles[0].name, filePrograss: 0, file: tempFiles[0], event: UploadFile, type: "image", idPatient: attrs.idpatient, changeName: attrs.changeName, staticname: attrs.staticname });
                                ...
                                if (scope.$root.$$phase) {
                                    scope.$emit('uploadFiles', { id: attrs.id, fileName: tempFiles[0].name, filePrograss: 0, file: tempFiles[0], event: UploadFile, type: ty, idPatient: attrs.idpatient, changeName: attrs.changeName, staticname: attrs.staticname });
                                }
                                else {
                                    scope.$apply(() => {
                                        scope.$emit('uploadFiles', { id: attrs.id, fileName: tempFiles[0].name, filePrograss: 0, file: tempFiles[0], event: UploadFile, type: ty, idPatient: attrs.idpatient, changeName: attrs.changeName, staticname: attrs.staticname });
                ...
                function UploadFile(number, img, fileErorr, newPath = null, changeName) {
                    element[0].querySelector(".textProsess").innerHTML = number + "%";
                    if (number == "C") {
                        scope.eventDone({ PATH: newPath });
