$ANContreoller.Auth_Login={
    FUN:['$scope','$cookies','$location',function($scope,$cookies,$location){
       ...
       //  console.log($cookies.remove('PULAB'));
        ...
        let Login=(res)=>{
            if(res.tempPass)
            {
                ...
                $cookies.put('PULAB', JSON.stringify({token:res.token,Did:res.user.dentalCenterId,name:res.user.nameDoctor,image:res.user.logo,phone:res.user.phoneNumber,email:res.user.email})); //PUA Parisaline User ACCountant
                $scope.$apply(()=>{
