...
$ANContreoller.Auth_Confirm={
    FUN:['$scope','$location','$cookies',function($scope,$cookies){
        ...
        $scope.init=()=>{
                    ...
                    HTTPs.POST(CONFIG.serverUrl+"/DCA",valueForm).then(res=>{
                        ...
                        if(res.message=="confirm Sucess")
                        {
                            $scope.$apply(()=>{
                                ...
                                $cookies.put('PULAB', JSON.stringify({token:$scope.PULAB.token,Did:res.user.dentalCenterId,name:res.user.nameDoctor,image:res.user.logo,phone:res.user.phoneNumber,email:res.user.email}));
                                l.stop();
                            });
                        }
                    });
                ...
                if($cookies.get("PULAB"))
                {
                    $scope.PUD=JSON.parse($cookies.get("PULAB"));
                }
