    ...
    this.$onInit = function () {
        ...
        $scope.getFile = () => {
            HTTPs.POST(CONFIG.serverUrl + "/GFP", { id: this.idPatient }).then(res => {
                ...
                if (res.message == 2000) {
                    $scope.$apply(() => {
                        if (res.fileSumalationPdf.length !== 0) {
                            $scope.fileSumlationPdf = res.fileSumalationPdf[0].path;
                        }
                        $scope.OptionNumber = res.numberStl[0].nLevel;
        ...
        $scope.eventDonUpload = (Path, name, type = null) => {
            if (type == "pts") {
                if ($scope.UpperArraypts.find(x => x.name + x.id == name)) {
                    $scope.UpperArraypts.find(x => x.name + x.id == name).Path = Path;
                }
                else if ($scope.LowerArraypts.find(x => x.name + x.id == name)) {
                    $scope.LowerArraypts.find(x => x.name + x.id == name).Path = Path;
                }
                return;
            }
            if ($scope.UpperArray.find(x => x.name + x.id == name)) {
                $scope.UpperArray.find(x => x.name + x.id == name).Path = Path;
            }
            else if ($scope.LowerArray.find(x => x.name + x.id == name)) {
                $scope.LowerArray.find(x => x.name + x.id == name).Path = Path;
            }
        }

        $scope.DeleteStl = (path, name, type = null) => {
            if (confirm("Are you sure you want to delete the file?")) {
                HTTPs.POST(CONFIG.serverUrl + "/DRP", { src: path.split(CONFIG.serverImageUrl)[1] || path }).then(res => {
                    if (res) {
                        $scope.$apply(() => {
                            if (type == "pts") {
                                if ($scope.UpperArraypts.find(x => x.name + x.id == name)) {
                                    $scope.UpperArraypts.find(x => x.name + x.id == name).Path = "";
                                }
                                else if ($scope.LowerArraypts.find(x => x.name + x.id == name)) {
                                    $scope.LowerArraypts.find(x => x.name + x.id == name).Path = "";
                                }
                            }
                            else {
                                if ($scope.UpperArray.find(x => x.name + x.id == name)) {
        ...
        $scope.showModelBarcode = (id, numberAliner, numberOfAliner, statusBarcode, numberOfAlinerLower, numberOfAlinerUpper, Upper = false, duble = false, alinerType, key = null) => {
            console.log(id, numberAliner, numberOfAliner, statusBarcode, numberOfAlinerLower, numberOfAlinerUpper, Upper, duble, alinerType, key);
        ...
        $scope.Loading = false;
        $scope.LinkStl = null;
        $scope.CompressAllFIleStlAndDownload = () => {
            $scope.Loading = true;
            HTTPs.POST(CONFIG.serverUrl + "/downloadAllFileStl", { patientId: this.idPatient }).then(res => {
                res = JSON.parse(res);
                if (res.error == 0) {
                    $scope.$apply(() => {
                        $scope.Loading = false;
                        $scope.LinkStl = res.data;
                    });
                }
            });
        }
